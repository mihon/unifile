...
class TreeDocumentFile extends UniFile {
    ...
    public UniFile findFile(String displayName) {
        ...
        // For Android storage providers, we assume document IDs are formed
        // based on filenames, allowing for a fast existence check.
        // For other providers, we fall back to the slower but more reliable
        // method of listing all files, as document ID structure is not guaranteed.
        //
        // Note on case sensitivity:
        // - Device Storage: Case sensitivity is determined by the document provider.
        //   For most Android storage this is typically case-insensitive.
        // - Other Providers: We perform an explicit case-insensitive
        //   search while looping through the directory. This ensures broad
        //   compatibility with providers that might have different or opaque
        //   naming schemes.
        if (DocumentsContractApi19.isDeviceStorage(mUri.getAuthority())) {
        ...
        for (NamedUri uri : result) {
            if (Utils.equals(displayName, uri.name, true)) {
